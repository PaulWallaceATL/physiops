...
export interface PreloaderProps {
  ...
  /** Optional second line of loading text (e.g. "Complete Autonomic Function Testing") */
  loadingTextLine2?: string;
...
const Preloader: React.FC<PreloaderProps> = ({
  ...
  const renderWord = (word: string, index: number, baseDelay: number) => (
    <motion.span
      key={`${word}-${index}`}
      ...
      transition={{
        ...
        delay: hideText ? 0 : baseDelay + index * 0.1,
        ease: [0.65, 0, 0.35, 1],
      }}
      className={cn(
  ...
  const renderLoadingText = () => {
    const line1Words = loadingText.split(" ");
    const line2Words = loadingTextLine2 ? loadingTextLine2.split(" ") : [];
    ...
    return (
      ...
      >
        <div className="flex flex-wrap justify-center gap-x-3 px-8">
          {line1Words.map((word, index) => renderWord(word, index, 0))}
        </div>
        {line2Words.length > 0 && (
          <div className="flex flex-wrap justify-center gap-x-3 px-8">
            {line2Words.map((word, index) =>
              renderWord(word, index, line1Words.length * 0.1),
            )}
          </div>
        )}
      </div>
